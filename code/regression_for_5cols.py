...
def init_global_var(all_img_flag_in=False,file_path_in=None,save_path_in=None,count=1):
    ...
    if count==1:    #如果是第一次初始化，则读取矩形框坐标对列表和图像大小
    ...
    while center_xy_copy:
        if i == 1:
            ...
            if first_point_flag:
                ...
                if len(points_for_regression) != first_col_num:
                    points_for_col_regression=[]
                    points_for_regression = first_col_line_points_for_regression.copy()  # 若第一列拟合点坐标列表长度不等于first_col_num，则用预设的first_col_line_points_for_regression代替
                    coordinates_for_first_col_regression=[]  # 预设的first_col_line_points_for_regression对应的矩形坐标列表
                    for kkk in range(first_col_num):
                        coordinates_for_first_col_regression.append(center_to_rectangle[first_col_line_points_for_regression[kkk]])  # 预设的first_col_line_points_for_regression对应的矩形坐标列表
                    coordinates_for_col_regression=coordinates_for_first_col_regression  # 预设的first_col_line_points_for_regression对应的矩形坐标列表
                    first_col_k,first_col_b=linear_regression(np.array(points_for_regression))  # 预设的first_col_line_points_for_regression对应的直线的斜率k和截距b
                    col_k_list[0]=first_col_k  # 预设的first_col_line_points_for_regression对应的直线的斜率k和截距b
                    col_b_list[0]=first_col_b  # 预设的first_col_line_points_for_regression对应的直线的斜率k和截距b
        ...
        for j in range(len(col_points)-1): #遍历该列的拟合点坐标，判断是否存在相交的情况
            ...
            if f :#存在相交的情况
                ...
                if j==len(col_points)-2: #若遍历到最后一个点，则退出循环
                    ...
                    continue
    if col_inter_flag==False and len(col_points)>nums_of_row: #若该列的拟合点坐标列表中点个数大于行数，但不存在相交的情况，则对相交面积占比大于0.005的矩形进行合并
...
#计算中心坐标
    ...
    #for i in range(len(points_for_col_regression)):
        #center_xy,points_for_col_regression[i]=merge_col_regression_intersect_rectangles(center_xy,points_for_col_regression[i],ax,position)
    #center_xy,points_for_col_regression[0]=merge_col_regression_intersect_rectangles(center_xy,points_for_col_regression[0],ax,position)   #合并第一列直线与矩形的交点,加上这行代码准确率为433（-5）/458   ,不加这行代码准确率为448（-5）/458
    #points_for_col_regression[0]=points_for_col_regression111
...
def merge_col_regression_intersect_rectangles(center_xy,points_for_per_col_regression,ax,position):
    ...
    :param ax: 子图ax
    :param position: 摄像头位置
    :return: center_xy,points_for_per_col_regression #返回中心点坐标列表和每列的拟合点坐标列表
    """
    col_points=points_for_per_col_regression 
    col_inter_flag=False
    end_flag=False
...
def merge_all_col_lines_rectangles(center_xy,points_for_col_regression):
    ...
    """
    for i in range(len(points_for_col_regression)):
        if len(points_for_col_regression[i])<6: #如果该列的拟合点数小于3,则认为该列的拟合结果不正确,删除该列的拟合点坐标列表
            center_xy,points_for_col_regression[i]=merge_col_regression_intersect_rectangles(center_xy,points_for_col_regression[i],ax,position)
    return center_xy,points_for_col_regression
...
def error_col_process():
    ...
    #print("所有行列拟合结果不正确,请检查")
        ...
        first_col_num,first_col_line_points_for_regression=fit_first_col_line_only(center_xy_cp,ax,position)
        points_for_col_regression=all_col_lines_regression(center_xy_cp,nums_of_col,position)
        center_xy,points_for_col_regression=merge_all_col_lines_rectangles(center_xy,points_for_col_regression)
        ...
        if sum(all_col_flags)==nums_of_col:
            ...
            missing_col_line_points() #尝试补充缺失的点
            center_xy,points_for_col_regression=merge_all_col_lines_rectangles(center_xy,points_for_col_regression)
            all_col_flags=[] #存储所有列的拟合结果是否正确
            for i in range(len(points_for_col_regression)):
                #print("第",i+1,"列的拟合点有：",len(points_for_col_regression[i]),"个坐标为：",points_for_col_regression[i])
...
if __name__ == '__main__':
    file_path__="../col_and_row_regression/test-data/labels/vacant2_1391.txt"
    ...
    #print("检测到的中心点数过少,疑似检测程序出错,请检查")
    global  first_col_num,first_col_line_points_for_regression #定义全局变量,用于存储第一列的点数和拟合点坐标列表
    first_col_num,first_col_line_points_for_regression=fit_first_col_line_only(center_xy_cp,ax,position) #只拟合最靠近摄像头一列的直线,获得第一列的点数
    ...
    if is_first_point_correct_flag==1 and first_col_num>=6 and len(center_xy_cp1)>=30 and len(points_for_col_regression_cp1)==5 and sum(col_flag_scores)==5: 
        ...
        if sum(all_col_flags)==nums_of_col:
            ...
            col_line_params,row_line_params,Matrix,points_to_matrix_dict=get_row_and_col_matrix(points_for_col_regression,points_for_row_regression,position) #计算行列矩阵
            display_colline_rowline_and_num( col_line_params[0],col_line_params[1],row_line_params[0],row_line_params[1],points_to_matrix_dict,center_xy,ax,img_size,points_for_col_regression) #绘制拟合直线和数字
            #draw_col_line(col_line_params[0],col_line_params[1],points_for_col_regression,ax,img_size) #绘制拟合直线
            put_text_of_camera_position(position,center_xy,ax,5) #绘制摄像头位置
            draw_center_point_and_rect(center_xy,ax,img_size) #绘制中心点和矩形框
            set_axis_and_save(1,ax,img_name,img_size,save_dir,save_flag)#设置坐标轴范围和保存路径等
            exit()
    ...
    #3.如果第一个点不正确,或第一列点数小于6,或中心点坐标个数大于等于30-10,或所有列的拟合结果不正确
        ...
        #for i in range(nums_of_col):  # 遍历每一列
            #print("第",i+1,"列的拟合点有：",len(points_for_col_regression[i]),"个坐标为：",points_for_col_regression[i])
        missing_col_line_points() #判断是否存在缺失的点,如果存在缺失的点,则尝试进行补充
        center_xy,points_for_col_regression=all_col_regression_check(points_for_col_regression,ax,nums_of_row,position) #检查每列直线拟合结果是否正确,合并
